...
| Kryo           | 序列化协议                                                   |
| Leaf-SnowFlake | 利用美团改进的雪花算法生成局部有序的消息id                   |

## 模块结构

| 模块             | 作用                                                         |
| ---------------- | ------------------------------------------------------------ |
| rim-client       | 客户端：负责接入IM系统、应用层ACK                            |
| rim-router       | 路由层：负责消息（群聊、单聊、离线）转发、用户回话信息管理、应用层ACK |
| rim-server       | 服务层：消息推送                                             |
| rim-store        | 存储层：消息（群聊、私聊）持久化、离线消息查询、应用层ACK    |
| rim-id-generator | ID生成层：生成群聊、单聊的消息id                             |
...
+ Dubbo泛化调用的地址为一致性哈希负载均衡算法计算所得
+ 解决了自定义协议在传输中导致的粘包、拆包问题

+ 群聊批量ACK处理，避免因创建过多的超时计时器导致的压力过大

+ 利用caffeine做zookeeper中服务器节点的本地缓存，并添加watch机制刷新缓存
+ 利用leaf-snowflake生成消息id
+ 如果客户端连接的服务器宕机，且有其他可用服务器，客户端可以实现无感切换（重连）
+ 使用时间轮算法实现了ACK等待列表功能
+ 服务端自动剔除离线用户
+ 三大特性
...
+ （已完成）路由层添加本地缓存，缓存zookeeper中注册的服务器信息，减少用户登陆时访问zookeeper的次数
+ （已完成）群聊、单聊ID生成器
+ 压测
...
+ 路由层：用caffeine本地缓存在zookeeper中注册的服务器节点，利用watch机制来监听节点变化，如果节点变化就用回调函数修改本地缓存。修改完后，对于用户的重连请求重新选择服务器连接。
